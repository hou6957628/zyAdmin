<template>
  <div class="content">
    <el-breadcrumb class="fs-16" separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/riskControl' }">额度流程管理</el-breadcrumb-item>
    </el-breadcrumb>
  ...
  export default {
    methods: {
      ...
      getProductList(data1,data2,data3){
        axios({
          method:"GET",
          url:"http://"+this.baseUrl+"/quota/admin/amountFlow/list",
          headers:{
        ...
        axios({
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            if (res.data.body == "true") {
              this.$alert('该额度流程在某应用（APP）引用，不可删除', '提示', {
                confirmButtonText: '确定',
                center: true,
                type: 'warning'
              });
            } else {
              this.$confirm('是否确认删除此额度流程?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning',
                center: true
      ...
      copyCustomerTag(formName){
        this.$refs[formName].validate((valid) => {
          if (valid) {
            axios({
              method:"post",
              url:"http://"+this.baseUrl+"/quota/admin/amountFlow/copy",
              headers:{
                'Content-Type':'application/x-www-form-urlencoded',
                'Authorization': localStorage.token
              },
      ...
      checkName2(rule, value, callback) {
        if (!value) {
          ...
          axios({
            method:"GET",
            url:"http://"+this.baseUrl+"/quota/admin/amountFlow/getByName",
            headers:{
              'Content-Type':'application/x-www-form-urlencoded',
              'Authorization': localStorage.token
            },
