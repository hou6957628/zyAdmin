<template>
  <div class="content">
    <el-breadcrumb class="fs-16" separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/tagList' }">用户标签分类列表</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="operationContent">
      <el-button class="upLoadBtn" @click="toTag()" type="primary">添加用户标签分类<i class="el-icon-upload el-icon-circle-plus"></i></el-button>
    </div>
    <template>
      <el-table
        :data="tableData"
  ...
  export default {
    methods: {
      ...
      handleCurrentChange(val) {
        this.getProductList(val,this.nowPageSizes,0,null);
      },
      //添加用户标签分类
      toTag(){
        this.$router.push({
      ...
      deleteProduct(row){
        axios({
          method:"get",
          url:"http://"+this.baseUrl+"/risk/admin/classification/getCustomerTagByClassId",
          headers:{
            'Content-Type':'application/x-www-form-urlencoded',
            'Authorization': localStorage.token
          },
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            if (res.data.body != 0) {
              this.$alert('该用户标签分类已被用户标签引用，不可删除', '提示', {
                confirmButtonText: '确定',
                center: true,
                type: 'warning'
              });
            } else {
              this.$confirm('是否确认删除此用户标签分类?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning',
                center: true
      ...
      obtainedProduct(row){
        axios({
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            if (res.data.body != 0) {
              this.$alert('该用户标签分类已被用户标签引用，不可停用', '提示', {
                confirmButtonText: '确定',
                center: true,
                type: 'warning'
              });
            } else {
              this.$confirm('是否确认停用此用户标签分类?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning',
                center: true
      ...
      obtainedCustomerTag(row){
        axios({
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            ...
            this.getProductList(1,20,0,null);
          }else {
            this.$message.error(res.data.msgInfo);
          }
        })
