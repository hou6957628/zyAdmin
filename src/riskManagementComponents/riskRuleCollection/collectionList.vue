<template>
  <div class="content">
    <el-breadcrumb class="fs-16" separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/ruleSetManagement' }">风控规则集管理</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="operationContent">
      <el-button class="upLoadBtn" @click="toAddProduct()" type="primary">创建规则集<i class="el-icon-upload el-icon-circle-plus"></i></el-button>
      <el-button class="upLoadBtn" @click="tolabelList()" type="primary">规则集类列表<i class="el-icon-upload el-icon-circle-plus"></i></el-button>
      <el-input @click="searchProduct" class="searchContent"
                placeholder="规则集名称、编号搜索"
                v-model="finProduct"
                clearable>
    ...
    <el-dialog
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="规则集名称:" prop="collectionName" :rules="[ruleForm.collectionName,{validator:checkName2, required: true, trigger:'blur'}]">
  ...
  export default {
    methods: {
      ...
      getProductList(data1,data2,data3,data4){
        axios({
          method:"GET",
          url:"http://"+this.baseUrl+"/risk/admin/rule_collection/list",
          headers:{
      ...
      copyCustomerTag(formName){
        this.$refs[formName].validate((valid) => {
          if (valid) {
            axios({
              method:"post",
              url:"http://"+this.baseUrl+"/risk/admin/rule_collection/copy",
              headers:{
                'Content-Type':'application/x-www-form-urlencoded',
                'Authorization': localStorage.token
              },
              params:{
                ...
                collectionDetail: this.ruleForm.collectionDetail,
      ...
      obtainedProduct(row){
        axios({
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            if (res.data.body.count != 0) {
              this.$alert('该规则集已被规则流程引用，不可停用', '提示', {
                confirmButtonText: '确定',
                center: true,
                type: 'warning'
              });
