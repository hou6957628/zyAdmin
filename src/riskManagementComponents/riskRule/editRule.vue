<template>
  <div class="content">
    <el-breadcrumb class="fs-16" separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/userTagsList' }">规则管理</el-breadcrumb-item>
    ...
    <div class="operationContent" style="width: 500px">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="规则名称:" prop="ruleName">
          <el-input v-model="ruleForm.ruleName">{{ruleForm.ruleName}}</el-input>
        </el-form-item>
        ...
        <el-form-item>
          <el-form-item class="labelList" v-for="(domain, index) in electDataList.domains" :key="index">
            ...
            <div v-if="domain.fieldDataType-1==0" style="display: inline-block;height: 1px">
              ...
              <el-input v-if="domain.symbolCode2" type="hidden" style="width: 1px;" v-model="domain.symbolCode=domain.symbolCode1+','+domain.symbolCode2"></el-input>
              <el-input v-if="!domain.symbolCode2" type="hidden" style="width: 1px;" v-model="domain.symbolCode=domain.symbolCode1"></el-input>
  ...
  export default {
    methods: {
        ...
        axios({
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            if (res.data.body.ruleCollectionCount != 0) {
              this.$confirm('修改规则，对应风控规则集也会发生改变，是否确定修改？', '提示', {
              ...
              this.$confirm('修改规则，对应风控流程也会发生改变，是否确定修改？', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                center: true,
                type: 'warning'
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.electDataList.domains[0].fieldValue==null | this.electDataList.domains[0].symbolCode==null | this.electDataList.domains[0].ruleAction==null) {
              ...
              axios({
                method:"POST",
                url:"http://"+this.baseUrl+"/risk/admin/rule/update",
                headers:{
      ...
      getRule(ruleId){
        axios({
          method:"get",
          url:"http://"+this.baseUrl+"/risk/admin/rule/id",
          headers:{
            'Content-Type':'application/x-www-form-urlencoded',
            'Authorization': localStorage.token
          },
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            this.ruleForm = res.data.body.rcRule;
            this.ruleForm.ruleName = res.data.body.rcRule.ruleName;
            this.ruleForm.ruleDetail = res.data.body.rcRule.ruleDetail;
            this.ruleForm.classifyId = res.data.body.rcRule.classifyId;
            this.ruleForm.enabled = res.data.body.rcRule.enabled;
            this.ruleForm.rule = res.data.body.rcRule.rule;
            this.ruleForm.ruleDesc = res.data.body.rcRule.ruleDesc;
            this.electDataList.domains=res.data.body.rcRuleItem;
            ...
            this.electDataList.domains.forEach(function (item,index) {
              ...
              if (item.fieldDataType==1 || item.fieldDataType==5) {
                if (item.symbolCode.indexOf(',')!=-1) {
                  ...
                  _this.electDataList.domains[index].symbolCode1=item.symbolCode;
