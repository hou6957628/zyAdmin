      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.status==11 && scope.row.outSourceStatus!=2" @click="cancelOutSource(scope.row)" type="text" size="medium">留案</el-button>
            <el-button v-if="scope.row.status==11 && scope.row.outSourceStatus==2" @click="confirmOutSource(scope.row)" type="text" size="medium">取消留案</el-button>
          </template>
        </el-table-column>
      </el-table>
    </template>
  ...
  export default {
    methods: {
        ...
        axios({
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            ...
            if (this.operationAdminGroup.groupRole == 1) {
              ...
              this.getProductList(1,30,null,null,null,null,null,null,null,null,this.groupId,this.adminId,1);
      ...
      searchContent(data){
        if (this.status==null) {
          this.getProductList(this.pageNum,this.pageSize,this.status,this.mobile,this.orderStartDate,this.orderEndDate,this.repaymentPaymentStartDate,
            this.repaymentPaymentEndDate,this.repaymentOverdueDay,this.productId,this.groupId,this.adminId,1);
        } else {
          this.getProductList(this.pageNum,this.pageSize,this.status,this.mobile,this.orderStartDate,this.orderEndDate,this.repaymentPaymentStartDate,
            this.repaymentPaymentEndDate,this.repaymentOverdueDay,this.productId,this.groupId,this.adminId,null);
        }
      },
      //每页显示多少条
      ...
      cancelOutSource(row) {
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/order/admin/borrowing/cancelOutSource",
          headers:{
            'Content-Type':'application/x-www-form-urlencoded',
            'Authorization': localStorage.token
          },
      ...
      //取消留案
      confirmOutSource(row) {
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/order/admin/borrowing/confirmOutSource",
          headers:{
            'Content-Type':'application/x-www-form-urlencoded',
            'Authorization': localStorage.token
          },
