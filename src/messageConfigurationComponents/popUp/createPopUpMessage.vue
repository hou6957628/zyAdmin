<template>
  <div class="content">
    <el-breadcrumb class="fs-16" separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/smsMessage' }">弹窗消息</el-breadcrumb-item>
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="160px" class="demo-ruleForm">
      ...
      <el-form-item style="margin-top: 20px" label="请选择位置：" prop="positionId">
        <el-select v-model="ruleForm.positionId" value-key="id" placeholder="请选择" @change="selectChange3($event,messagePositionList)">
          <el-option
            v-for="item in messagePositionList"
            :key="item.id"
      ...
      <el-form-item label="请输入发送次数：" prop="popupCount">
        <el-input v-model="ruleForm.popupCount"></el-input>
      </el-form-item>
      <el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      getPositionList() {
        axios({
          method:"POST",
          url:"http://"+this.baseUrl+"/message/admin/message_position/findList",
          headers:{
            'Content-Type':'application/x-www-form-urlencoded',
            'Authorization': localStorage.token
          }
          ...
          if(res.data.msgCd=='ZYCASH-200'){
            this.messagePositionList=res.data.body;
          }else if(res.data.msgCd=='ZYCASH-1009'){
            this.$message.error(res.data.msgInfo);
          }
          else {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            param.append('positionId', this.ruleForm.positionId);
            param.append('positionName', this.ruleForm.positionName);
      ...
      //封装位置名称
      selectChange3(vId,list){
        let obj = {};
        obj = list.find((item)=>{
          return item.id === vId;
        });
        this.ruleForm.positionName=obj.positionName;
      },
      //取消按钮
      resetForm(formName) {
        this.$router.go(-1);
